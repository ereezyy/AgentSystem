...
class PricingRecommendationRequest(BaseModel):
    ...
    strategy: PricingStrategyAPI = Field(
        default=PricingStrategyAPI.DYNAMIC, description="Pricing strategy to use"
    )
...
class TierOptimizationRequest(BaseModel):
    ...
    tiers_to_optimize: Optional[List[PricingTierAPI]] = Field(
        None, description="Specific tiers to optimize"
    )
...
class PricingExperimentRequest(BaseModel):
    ...
    duration_days: int = Field(
        ..., ge=7, le=90, description="Experiment duration in days"
    )
...
class MarketConditionsUpdate(BaseModel):
    competitive_pressure: float = Field(
        ..., ge=0, le=1, description="Competitive pressure (0-1)"
    )
    market_growth_rate: float = Field(
        ..., ge=-1, le=1, description="Market growth rate"
    )
    customer_acquisition_cost: float = Field(
        ..., ge=0, description="Customer acquisition cost"
    )
...
class CompetitiveIntelligenceRequest(BaseModel):
    ...
    features_included: List[str] = Field(
        default_factory=list, description="Features included"
    )
    ...
    confidence_score: float = Field(
        default=0.8, ge=0, le=1, description="Data confidence"
    )
...
@router.post(
    "/recommendations/batch", response_model=List[PricingRecommendationResponse]
)
async def generate_batch_pricing_recommendations(
    ...
    try:
        ...
        if len(request.customer_ids) > 1000:
            raise HTTPException(
                status_code=400, detail="Maximum 1000 customers per batch"
            )
...
async def get_pricing_recommendations(
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            return [
                PricingRecommendationResponse(
                    ...
                    supporting_metrics=(
                        json.loads(row["supporting_metrics"])
                        if row["supporting_metrics"]
                        else {}
                    ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get pricing recommendations: {str(e)}"
        )
...
async def implement_pricing_recommendation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to implement recommendation: {str(e)}"
        )


@router.get(
    "/customers/{customer_id}/profile", response_model=CustomerPricingProfileResponse
)
async def get_customer_pricing_profile(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get customer profile: {str(e)}"
        )
...
async def optimize_pricing_tiers(
    ...
    try:
        ...
        optimization_result = await pricing_engine.optimize_pricing_tiers(
            tenant_id=tenant_id, target_metrics=request.target_metrics
        )
        return TierOptimizationResponse(**optimization_result)
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to optimize pricing tiers: {str(e)}"
        )
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            result = await conn.fetchrow(
                ...
                request.test_price,
                request.control_price,
                start_date,
                end_date,
                request.sample_size,
                json.dumps(request.success_metrics),
                "running",
            )
            ...
            return PricingExperimentResponse(
                ...
                statistical_significance=(
                    float(result["statistical_significance"])
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create pricing experiment: {str(e)}"
        )
...
async def get_pricing_experiments(
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            return [
                PricingExperimentResponse(
                    ...
                    statistical_significance=(
                        float(row["statistical_significance"])
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get pricing experiments: {str(e)}"
        )
...
async def get_pricing_dashboard(
    tenant_id: UUID = Depends(get_current_tenant), token: str = Depends(security)
):
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            return PricingDashboardResponse(
                ...
                total_revenue_opportunity=float(
                    result["total_revenue_opportunity"] or 0
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get pricing dashboard: {str(e)}"
        )
...
async def get_pricing_performance(
    ...
    try:
        ...
        performance = await pricing_engine.monitor_pricing_performance(
            tenant_id=tenant_id, days_back=days_back
        )
        return performance
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get pricing performance: {str(e)}"
        )
...
async def get_price_elasticity(
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            return [
                PriceElasticityResponse(
                    ...
                    pricing_tier=(
                        PricingTierAPI(row["pricing_tier"])
                        if row["pricing_tier"]
                        else None
                    ),
                    elasticity_coefficient=float(row["elasticity_coefficient"]),
                    confidence_interval=(
                        json.loads(row["confidence_interval"])
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get price elasticity: {str(e)}"
        )
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            result = await conn.fetchrow(
                query,
                tenant_id,
                request.competitive_pressure,
                request.market_growth_rate,
                request.customer_acquisition_cost,
                request.average_deal_size,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update market conditions: {str(e)}"
        )


@router.post(
    "/competitive-intelligence", response_model=CompetitiveIntelligenceResponse
)
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            result = await conn.fetchrow(
                ...
                request.price,
                json.dumps(request.features_included),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add competitive intelligence: {str(e)}"
        )


@router.get(
    "/competitive-intelligence", response_model=List[CompetitiveIntelligenceResponse]
)
async def get_competitive_intelligence(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get competitive intelligence: {str(e)}"
        )
...
async def get_pricing_alerts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get pricing alerts: {str(e)}"
        )
...
async def calculate_price_elasticity(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to calculate price elasticity: {str(e)}"
        )
