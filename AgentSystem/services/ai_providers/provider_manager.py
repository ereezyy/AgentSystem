...
from .local_provider import LocalProvider, create_local_provider
...
class ProviderType(Enum):
    ...
    LOCAL = "local"  # Local or open-source models
...
class AIProviderManager:
    ...
    def _load_configuration(self, config_file: Optional[str] = None):
        ...
        if local_model_path or os.getenv("LOCAL_API_ENDPOINT"):
            configs.append(ProviderConfig(
                provider_type=ProviderType.LOCAL,
                api_key="",  # No API key needed for local
                priority=int(os.getenv("LOCAL_PRIORITY", "4")),  # Lowest priority as fallback
        ...
        for provider_type, config in self.provider_configs.items():
            ...
            try:
                if provider_type == ProviderType.OPENROUTER:
                    ...
                    self.providers[provider_type] = create_local_provider(
