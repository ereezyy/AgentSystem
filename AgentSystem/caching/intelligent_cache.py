...
class IntelligentCache:
    ...
    async def get_cached_response(
        ...
        exact_hash = self._generate_exact_hash(
            request_content, model, temperature, max_tokens
        )
        ...
        if cache_hit:
            await self._update_access_stats(
                cache_hit.cache_entry, CacheLevel.EXACT_MATCH
            )
        ...
        if self.embedding_model and semantic_hash:
            cache_hit = await self._check_semantic_match(
                tenant_id, semantic_hash, request_content
            )
            if cache_hit:
                await self._update_access_stats(
                    cache_hit.cache_entry, CacheLevel.SEMANTIC_MATCH
                )
                self._track_performance(tenant_id, True, time.time() - start_time)
                return cache_hit
        ...
        if cache_hit:
            await self._update_access_stats(
                cache_hit.cache_entry, CacheLevel.PARTIAL_MATCH
            )
            self._track_performance(tenant_id, True, time.time() - start_time)
            return cache_hit
        ...
        if cache_hit:
            await self._update_access_stats(
                cache_hit.cache_entry, CacheLevel.TEMPLATE_MATCH
            )
            self._track_performance(tenant_id, True, time.time() - start_time)
            return cache_hit
    ...
    async def store_response(
        ...
        logger.debug(
            f"Stored cache entry {cache_entry.cache_id} for tenant {tenant_id}"
        )
    ...
    async def invalidate_cache(
        ...
        logger.info(
            f"Invalidated {invalidated_count} cache entries for tenant {tenant_id}"
        )
    ...
    def _generate_exact_hash(
        self, content: str, model: str, temperature: float, max_tokens: int
    ) -> str:
    ...
    async def _check_exact_match(
        self, tenant_id: str, exact_hash: str
    ) -> Optional[CacheHit]:
    ...
    async def _check_semantic_match(
        self, tenant_id: str, semantic_hash: str, content: str
    ) -> Optional[CacheHit]:
        ...
        candidate_keys = keys[:50]
        if not candidate_keys:
            return None

        # Optimization: Use MGET to fetch all candidates in one round trip
        cached_results = await self.redis.mget(*candidate_keys)
                ...
                try:
                    ...
                    if self.config["compression_enabled"]:
                        try:
                            ...
                            # Not gzipped, proceed with original data
                            pass
                    ...
                    similarity = await self._calculate_similarity(
                        content, cache_entry.request_content
                    )
    ...
    async def _check_partial_match(
        self, tenant_id: str, content: str, model: str
    ) -> Optional[CacheHit]:
    ...
    async def _check_template_match(
        self, tenant_id: str, content: str, model: str
    ) -> Optional[CacheHit]:
    ...
    async def _store_in_redis(self, cache_entry: CacheEntry):
        ...
        semantic_key = (
            f"cache:semantic:{cache_entry.tenant_id}:{cache_entry.semantic_hash}"
        )
    ...
    async def _store_in_database(self, cache_entry: CacheEntry):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            """,
                ...
                cache_entry.tenant_id,
                cache_entry.request_hash,
                cache_entry.semantic_hash,
                cache_entry.input_tokens,
                cache_entry.output_tokens,
                cache_entry.model_used,
                cache_entry.prompt_template,
                cache_entry.request_content,
                cache_entry.response_content,
    ...
    async def _update_access_stats(
        self, cache_entry: CacheEntry, cache_level: CacheLevel
    ):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            """,
                ...
                cache_entry.cost_saved,
                50,
            )  # Approximate cache response time
    ...
    async def _check_cache_limits(self, tenant_id: str) -> bool:
        ...
        return (
            stats["entry_count"] >= self.config["max_entries_per_tenant"]
            or stats["size_mb"] >= self.config["max_cache_size_mb"]
        )
    ...
    async def _load_cache_entry(self, cache_id: str) -> Optional[CacheEntry]:
        ...
        if entry_data:
            return CacheEntry(
                ...
                metadata=(
                    json.loads(entry_data["metadata"]) if entry_data["metadata"] else {}
                ),
    ...
    async def _mark_invalidated_in_database(
        self,
        tenant_id: str,
        pattern: Optional[str] = None,
        model: Optional[str] = None,
        ...
        query = (
            "UPDATE caching.cache_entries SET is_active = false WHERE tenant_id = $1"
        )
        params = [tenant_id]
        param_count = 1
...
class CacheManager:
    ...
    async def get_cached_response(
        self, tenant_id: str, request_content: str, model: str, **kwargs
    ) -> Optional[CacheHit]:
